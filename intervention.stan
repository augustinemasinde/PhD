...
transformed parameters {
  ...
  if (K == 1) {
    // single phase: fill lambda with the single annual_foi[K]
    for (j in 1:A) {
      lambda[j] = annual_foi[K];
    }
  } else {
    // For i = 2..K, use T[i-1] as the duration of the previous phase.
    for (i in 2:K) {
      Time[i] = Time[i-1] + T[i-1];
    }
    // Now assign lambda[j] by checking intervals Time[i]..Time[i+1]
    for (j in 1:A) {
      // default: if j >= Time[K] --> last phase
      if (j >= Time[K]) {
        lambda[j] = annual_foi[K];
      } else {
        // search for the phase i such that j in [Time[i], Time[i+1])
        for (i in 1:(K-1)) {
  ...
  // Safety: if K==1, T[1] may be defined but unused; if K>1, T[K] is not used as a change-point.
  // Keep T[K] present for compatibility (value already set above), but it is not used to define Time.
  // --------------------------------------------------
...
model {
  ...
  if (K > 1) {
    ...
    // K == 1: no T_raw priors required (entries may exist but are unused)
  }

  // Priors for annual_foi_raw for all K phases
  for (i in 1:K) {
    if (prior_distribution_constant_foi == 1) {
