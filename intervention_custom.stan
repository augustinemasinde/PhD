...
data {
  ...
  array[A,NAgeGroups] int <lower=1> class2 ; //upper boundary
  ...
  int <lower=1> K; 
  array[K] real priorT1;             // note: we'll use priorT1[1:(K-1)]
  array[K] real <lower = 0> priorT2; // same
  array[K] real <lower = 0> priorC1; // now per-phase priorC1
  array[K] real <lower = 0> priorC2; // now per-phase priorC2
...
transformed parameters {
  ...
  for(i in 1:K){
    ...
    if(prior_distribution_constant_foi == 1){
      // per-phase log-normal style: median = priorC1[i], sd(log) = priorC2[i]
      annual_foi[i] = priorC1[i] * exp(priorC2[i] * annual_foi_raw[i]) ;
    }   
  ...
  // --------- Time construction (Option A) ----------
  // Use the first K-1 T's as change-point durations.
  // Time[1] is the start of the age index (1). Then Time[2] = Time[1] + T[1] etc.
  Time[1] = 1;
  ...
  if (K == 1) {
    ...
    for (i in 2:K) {
      // use T[i-1] as the duration of the previous phase.
      Time[i] = Time[i-1] + T[i-1];
    }
    for (j in 1:A) {
      if (j >= Time[K]) {
        lambda[j] = annual_foi[K];
  ...
  // Safety note: T[K] exists (for compatibility) but is not used as a change-point duration.
  // --------------------------------------------------
...
model {
  // Priors for T_raw: only the first K-1 T_raw values correspond to real change-point durations.
  if (K > 1) {
    for (i in 1:(K-1)) {
      if (prior_distribution_T == 1) { // normal
        T_raw[i] ~ normal(0, 1);
      }
      if (prior_distribution_T == 2) { // exponential
        T_raw[i] ~ exponential(priorT1[i]);
      }
    }
  }

  // Priors for annual_foi_raw for all K phases (per-phase priors supported)
  for (i in 1:K) {
    if (prior_distribution_constant_foi == 1) {
      annual_foi_raw[i] ~ normal(0, 1);
    }   
    if (prior_distribution_constant_foi == 2) {
      annual_foi_raw[i] ~ exponential(priorC1[i]);
    }
  }
